...
const { sendWelcomeEmail, sendNewLeadNotification, testEmailConfig } = require('../services/email');
...
router.get('/test', (req, res) => {
    res.json({
        ...
        endpoints: [
            'GET /api/leads/test - This endpoint',
    ...
    try {
        ...
        res.json({
            ...
            config: {
                ...
                business_email: process.env.BUSINESS_EMAIL ? 'configured' : 'missing'
            },
            timestamp: new Date().toISOString()
        });
    } catch (error) {
...
router.post('/capture', async (req, res) => {
    try {
        ...
        try {
            ...
            // No database connection - return test mode response
            console.log('⚠️ Test mode: Lead captured without database');
